import math
from ShrutiMusic.utils.formatters import time_to_seconds
from ShrutiMusic.utils.inline.buttons import inline
from config import BOT_USERNAME, SUPPORT_GROUP, SUPPORT_CHANNEL


def track_markup(_, videoid, user_id, channel, fplay):
    # inline.ikm hata kar sirf normal list banayi hai
    buttons = [
        [
            inline._button(
                text=_["P_B_1"],
                category="primary",
                callback_data=f"MusicStream {videoid}|{user_id}|a|{channel}|{fplay}",
            ),
            inline._button(
                text=_["P_B_2"],
                category="success",
                callback_data=f"MusicStream {videoid}|{user_id}|v|{channel}|{fplay}",
            ),
        ],
        [
            inline._button(
                text=_["CLOSE_BUTTON"],
                category="danger",
                callback_data=f"forceclose {videoid}|{user_id}",
            )
        ],
    ]
    return buttons


def stream_markup_timer(_, chat_id, played, dur):
    played_sec = time_to_seconds(played)
    duration_sec = time_to_seconds(dur)
    percentage = (played_sec / duration_sec) * 100
    umm = math.floor(percentage)
    if 0 < umm <= 10:
        bar = "◉—————————"
    elif 10 < umm < 20:
        bar = "—◉————————"
    elif 20 <= umm < 30:
        bar = "——◉———————"
    elif 30 <= umm < 40:
        bar = "———◉——————"
    elif 40 <= umm < 50:
        bar = "————◉—————"
    elif 50 <= umm < 60:
        bar = "—————◉————"
    elif 60 <= umm < 70:
        bar = "——————◉———"
    elif 70 <= umm < 80:
        bar = "———————◉——"
    elif 80 <= umm < 95:
        bar = "————————◉—"
    else:
        bar = "—————————◉"

    # Yahan se bhi inline.ikm wrapper hata diya, ab skip error nahi aayega!
    buttons = [
        # Progress bar button
        [
            inline._button(
                text=f"{played} {bar} {dur}",
                category="primary",
                url=f"https://t.me/{BOT_USERNAME}?startgroup=true"
            )
        ],
        # Control buttons
        [
            inline._button(
                text="▷",
                category="success",
                callback_data=f"ADMIN Resume|{chat_id}",
            ),
            inline._button(
                text="II",
                category="success",
                callback_data=f"ADMIN Pause|{chat_id}",
            ),
            inline._button(
                text="↻",
                category="success",
                callback_data=f"ADMIN Replay|{chat_id}",
            ),
            inline._button(
                text="‣‣I",
                category="success",
                callback_data=f"ADMIN Skip|{chat_id}",
            ),
            inline._button(
                text="▢",
                category="success",
                callback_data=f"ADMIN Stop|{chat_id}",
            ),
        ],
        # Support & Channel
        [
            inline._button(
                text="𝚂𝚄𝙿𝙿𝙾𝚁𝚃",
                category="success",
                url=SUPPORT_GROUP,
                icon_custom_emoji_id="6298522513987078458",
            ),
            inline._button(
                text="𝙲𝙷𝙴𝙽𝙽𝙴𝙻",
                category="primary",
                url=SUPPORT_CHANNEL,
                icon_custom_emoji_id="6066562059158429517",
            ),
        ],
        # Close button
        [
            inline._button(
                text=_["CLOSE_BUTTON"],
                category="danger",
                callback_data="close",
            )
        ],
    ]
    return buttons


def stream_markup(_, chat_id):
    buttons = [
        [
            inline._button(
                text="▷",
                category="primary",
                callback_data=f"ADMIN Resume|{chat_id}",
            ),
            inline._button(
                text="II",
                category="primary",
                callback_data=f"ADMIN Pause|{chat_id}",
            ),
            inline._button(
                text="↻",
                category="primary",
                callback_data=f"ADMIN Replay|{chat_id}",
            ),
            inline._button(
                text="‣‣I",
                category="primary",
                callback_data=f"ADMIN Skip|{chat_id}",
            ),
            inline._button(
                text="▢",
                category="danger",
                callback_data=f"ADMIN Stop|{chat_id}",
            ),
        ],
        [
            inline._button(
                text=_["CLOSE_BUTTON"],
                category="danger",
                callback_data="close",
            )
        ],
    ]
    return buttons


def playlist_markup(_, videoid, user_id, ptype, channel, fplay):
    buttons = [
        [
            inline._button(
                text=_["P_B_1"],
                category="primary",
                callback_data=f"NandPlaylists {videoid}|{user_id}|{ptype}|a|{channel}|{fplay}",
            ),
            inline._button(
                text=_["P_B_2"],
                category="success",
                callback_data=f"NandPlaylists {videoid}|{user_id}|{ptype}|v|{channel}|{fplay}",
            ),
        ],
        [
            inline._button(
                text=_["CLOSE_BUTTON"],
                category="danger",
                callback_data=f"forceclose {videoid}|{user_id}",
            )
        ],
    ]
    return buttons


def livestream_markup(_, videoid, user_id, mode, channel, fplay):
    buttons = [
        [
            inline._button(
                text=_["P_B_3"],
                category="primary",
                callback_data=f"LiveStream {videoid}|{user_id}|{mode}|{channel}|{fplay}",
            ),
        ],
        [
            inline._button(
                text=_["CLOSE_BUTTON"],
                category="danger",
                callback_data=f"forceclose {videoid}|{user_id}",
            )
        ],
    ]
    return buttons


def slider_markup(_, videoid, user_id, query, query_type, channel, fplay):
    buttons = [
        [
            inline._button(
                text=_["P_B_1"],
                category="primary",
                callback_data=f"MusicStream {videoid}|{user_id}|a|{channel}|{fplay}",
            ),
            inline._button(
                text=_["P_B_2"],
                category="success",
                callback_data=f"MusicStream {videoid}|{user_id}|v|{channel}|{fplay}",
            ),
        ],
        [
            inline._button(
                text="◁",
                category="primary",
                callback_data=f"slider B|{query_type}|{query}|{user_id}|{channel}|{fplay}",
            ),
            inline._button(
                text=_["CLOSE_BUTTON"],
                category="danger",
                callback_data=f"forceclose {query}|{user_id}",
            ),
            inline._button(
                text="▷",
                category="primary",
                callback_data=f"slider F|{query_type}|{query}|{user_id}|{channel}|{fplay}",
            ),
        ],
    ]
    return buttons
